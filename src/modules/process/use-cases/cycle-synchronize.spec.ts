...
describe('Process use case test', () => {
    it('Should execute partial sync and return a response dto', async () => {
        ...
        jest.spyOn(synchronizeService, 'synchronizeCycle')
        ...
        // THEN
        expect(synchronizeService.synchronizeCycle).toHaveBeenLastCalledWith(synchronizeCycleModel);
        expect(synchronizeService.synchronizeCycle).toBeTruthy();
        expect(synchronizeService.synchronizeCycle).toHaveBeenCalledTimes(1);
    });
});
